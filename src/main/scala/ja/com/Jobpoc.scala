...
object Jobpoc {
  ...
  def main(args: Array[String]): Unit = {       // 2
    ...
    // this is working
    // AWS - S3 region -
   /* val test1 = JobSparkConf.sc.parallelize(List(1, 2, 3))
    test1.saveAsTextFile("s3://estestoutput1/sourceloc/test1203.txt")
    val test2 = JobSparkConf.sc.textFile("s3://estestoutput1/sourceloc/test1203.txt")
    println("*************count=" + test2.count())*/
...
//   var arcfile =  "C:\\Users\\Ja\\Downloads\\pptdocsrcfile\\BL-100267.arc.gz" // s3://estestoutput1/sourceloc/BL-100267.cdx  C:\Users\Ja\Downloads\pptdocsrcfile\BL-100267.cdx
//    var cdxfile = "C:\\Users\\Ja\\Downloads\\pptdocsrcfile\\BL-100267.cdx"    // C:\Users\Ja\Downloads\pptdocsrcfile\BL-100267.arc.gz


    var arcfile = "C:\\Users\\Ja\\Downloads\\oldsrcfiles\\IM_TNA_monthly_042016_www.open-water.org.uk-20160415110019-00000.arc.gz" //give  thek full ofile  name
    var cdxfile = "C:\\Users\\Ja\\Downloads\\oldsrcfiles\\IM_TNA_monthly_042016_www.open-water.org.uk-20160415110019-00000.cdx"
    /*
    var arcfile = "C:\\Users\\Ja\\Downloads\\pptdocsrcfile\\arcfiles\\BL-100267.arc.gz" //give  thek full ofile  name
    var cdxfile = "C:\\Users\\Ja\\Downloads\\pptdocsrcfile\\cdxfiles\\BL-100267.cdx"
*/
   // var dnsfiles = "/whiltelist/dns_*" //ToDO: restore the correct settings linked to AWS
    var dnsfiles = "C:\\Users\\Ja\\Google Drive\\oldfiles\\Note\\srcfile\\SearchWhitelist\\dns_*"
    //  var username = "edureka"
...
arcfile = "C:\\Users\\Ja\\Downloads\\pptdocsrcfile\\arcfiles\\BL-100267.arc.gz" //give  thek full ofile  name
cdxfile = "C:\\Users\\Ja\\Downloads\\pptdocsrcfile\\cdxfiles\\BL-100267.cdx"
dnsfiles = "C:\\Users\\Ja\\Google Drive\\oldfiles\\Note\\srcfile\\SearchWhitelist\\dns_*"
   ...
   // val file = new File(arcfile)                                 //  val file1 = new File(arcfile)   thinking about hadoop files..this will work for local .. not for hadoop
    ...
    val lstFinalDataToSave = joinedDF.collect().map(cdxItem => {
      ...
      try {
         ...
         else if(mime.toLowerCase.contains("opendocument"))
         {
           imageData =  jatikaparser.parseOdfFile(bodyBytes).replaceAll("\\s{2,}", " ").trim
         }
         else if(mime.equals("application/msword") || mime.equals("application/vnd.ms-excel") || mime.contains("vnd.ms-powerpoint")   )
           {
             val xtype = url.endsWith("x")
            // println("********************************************************************")

             /*Hyper Text Markup Language (HTM, HTML, XHTML
               Dynamically generated content (e.g.asp, aspx, php and jsp etc.)
             ...
             Text files (TXT) - but provide the option to exclude robots.txt, .js and, .json extensions, and any files which returned an HTTP status code of 400
             */
...
//1 sec sir .. complete the final step
   ...
   val fDf1 = rdd.toDF().select("length","language","url","textTitle","detectedMime",
    ...
    val fDf_demo1 = fDf1.select("length","language","url","textTitle","detectedMime",
      "timestamp",     "indexingTime",     "textContent",     "digest").filter(fDf1("detectedMime").contains("pdf"))
    ...
    val fDf_demo2 = fDf1.select("length","language","url","textTitle","detectedMime",
      "timestamp",     "indexingTime",     "textContent",     "digest").filter(fDf1("detectedMime").contains("msword"))
    ...
    val fDf_demo3 = fDf1.select("length","language","url","textTitle","detectedMime",
      "timestamp",     "indexingTime",     "textContent",     "digest").filter(fDf1("detectedMime").contains("html"))
    fDf_demo3.rdd.coalesce(1).saveAsTextFile("C:\\Users\\Ja\\Google Drive\\srcfile\\esoutput_json\\html_html_output_"  + format.format(new java.util.Date()))


    val fDf_demo4 = fDf1.select("length","language","url","textTitle","detectedMime",
      "timestamp",     "indexingTime",     "textContent",     "digest").filter(fDf1("detectedMime").contains("vnd.ms-excel"))
    fDf_demo4.rdd.coalesce(1).saveAsTextFile("C:\\Users\\Ja\\Google Drive\\srcfile\\esoutput_json\\vnd_ms-excel_vnd_ms-excel_"  + format.format(new java.util.Date()))

    val fDf_demo5 = fDf1.select("length","language","url","textTitle","detectedMime",
      "timestamp",     "indexingTime",     "textContent",     "digest").filter(fDf1("detectedMime").contains("plain"))
    fDf_demo5.rdd.coalesce(1).saveAsTextFile("C:\\Users\\Ja\\Google Drive\\srcfile\\esoutput_json\\plain_plain_output_"  + format.format(new java.util.Date()))

    val fDf_demo6 = fDf1.select("length","language","url","textTitle","detectedMime",
      "timestamp",     "indexingTime",     "textContent",     "digest").filter(fDf1("detectedMime").contains("vnd.ms-powerpoint"))
    fDf_demo6.rdd.coalesce(1).saveAsTextFile("C:\\Users\\Ja\\Google Drive\\srcfile\\esoutput_json\\vnd_ms-powerpoint_vnd.ms-powerpoint_output_"  + format.format(new java.util.Date()))


    val fDf_demo7 = fDf1.select("length","language","url","textTitle","detectedMime",
      "timestamp",     "indexingTime",     "textContent",     "digest").filter(fDf1("detectedMime").contains("octet-stream"))
    fDf_demo7.rdd.coalesce(1).saveAsTextFile("C:\\Users\\Ja\\Google Drive\\srcfile\\esoutput_json\\octet-stream_octet-stream_output_"  + format.format(new java.util.Date()))
    ...
    */
    ...
    val validationOutputs = new mutable.HashMap[String, mutable.StringBuilder]()
